...
## 2026-05-30 — v0.5.0 release artifact smoke

- **Host:** `128.140.3.159`
- **OS:** Hetzner Ubuntu VPS, `x86_64`
- **Release tested:** `v0.5.0`
- **Smoke root:** `/tmp/simple-vps-release-smoke-Nnrjot`
- **DNS/TLS:** `nip.io` hostname with `tls = "internal"` and curl
  `--resolve ... -k`

Purpose: verify the stable published `v0.5.0` release assets install the host
and deploy a generated first app.
...
release: v0.5.0
host: 128.140.3.159
app: svps-smoke-080940
route host: svps-smoke-080940.128.140.3.159.nip.io
==> Downloading Simple VPS binary from https://github.com/fprl/simple-vps/releases/download/v0.5.0/simple-vps-darwin-arm64
==> Downloading Simple VPS Linux helper binary from https://github.com/fprl/simple-vps/releases/download/v0.5.0/simple-vps-linux-amd64
==> Apply 20260530T081059Z changed 1 operations
v0.5.0
Deployed svps-smoke-080940 (production) at 2979517762d4
/health -> ok
/       -> {"app":"svps-smoke-080940","status":"running","database_path":"/data/app.sqlite"}
Destroyed svps-smoke-080940 (production)
...
**Outcome:** pass. The stable release installer and assets can install a host,
generate a PHP app with `simple-vps init`, deploy, serve through Caddy,
...
- **Build tested:** local post-`v0.5.0-rc2` build with `simple-vps init`
  template changes.
- **Smoke root:** `/tmp/simple-vps-init-real-fC6rL4`
- **Temporary host:** `init-php.128.140.3.159.nip.io`
- **DNS/TLS:** `tls = "internal"` generated by `simple-vps init --tls
  internal`, curl with `--resolve ... -k`
...
simple-vps init \
  ...
  --host init-php.128.140.3.159.nip.io \
  --tls internal
git init && git add . && git commit -m "init php smoke"
simple-vps check --env production
simple-vps setup --env production
simple-vps deploy --env production
curl -k --resolve init-php.128.140.3.159.nip.io:443:128.140.3.159 \
  https://init-php.128.140.3.159.nip.io/health
curl -k --resolve init-php.128.140.3.159.nip.io:443:128.140.3.159 \
  https://init-php.128.140.3.159.nip.io/
simple-vps destroy --env production --confirm init-php --purge
```

Checks:

- `init` created `simple-vps.toml`, `Dockerfile`, and `public/index.php`.
- `check --env production` passed.
- Deploy succeeded at release `f144643948db`.
- HTTPS `/health` returned `ok`.
- HTTPS `/` returned JSON containing `"app":"init-php"` and
  `"database_path":"/data/app.sqlite"`.
- Final `app list --server deploy@128.140.3.159 --json` returned
  `{"apps":[]}`.

**Outcome:** pass. An app generated by `simple-vps init --template php` can be
committed, deployed, served through Caddy, and destroyed on the real VPS.
...
## 2026-05-29 — `simple-vps init` PHP Template Smoke

VPS: Hetzner Ubuntu 24.04 at `128.140.3.159`.

Purpose: verify that a project generated by `simple-vps init --template php`
passes the current `--env` CLI contract and deploys on the real host.

Steps run from a fresh temp app directory:

```sh
simple-vps init \
  --template php \
  --name init-php-current \
  --server deploy@128.140.3.159 \
  --host init-php-current.128.140.3.159.nip.io \
  --tls internal
git init
git add .
git commit -m "init php current smoke"
simple-vps check --env production
simple-vps setup --env production
simple-vps deploy --env production
curl -ksS --resolve init-php-current.128.140.3.159.nip.io:443:128.140.3.159 \
  https://init-php-current.128.140.3.159.nip.io/health
curl -ksS --resolve init-php-current.128.140.3.159.nip.io:443:128.140.3.159 \
  https://init-php-current.128.140.3.159.nip.io/
simple-vps destroy --env production --confirm init-php-current --purge
...
```text
Deployed init-php-current (production) at 072783fd18cb
/health -> ok
/       -> {"app":"init-php-current","status":"running","database_path":"/data/app.sqlite"}
Destroyed init-php-current (production)
app list --json -> {"apps":[]}
```
